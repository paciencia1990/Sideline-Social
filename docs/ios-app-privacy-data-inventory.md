...
| Other diagnostic data | Firebase operational/service metadata requires provider review; Coach AI is disabled and creates no current request/provider records | App functionality/security if provider-retained | Provider-dependent | No | Do not classify dormant Coach AI as current collection |
...
4. Coach AI is disabled for production and its provider implementation is isolated. Before any future enablement, identify and approve the processor, update disclosures and consent as required, and prohibit personal/child data in prompts.
