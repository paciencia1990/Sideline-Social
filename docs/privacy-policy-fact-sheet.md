...
- The source supports separate controlled free staging and production-connected beta profiles. Each requires the exact testing flag and its own mutually exclusive environment marker, a refreshed administrator-issued tester claim, signed-in adult Coach Mode, active standing, the server deployment flag, and a fail-closed runtime circuit breaker. The normal production profile contains no testing or beta flag and remains hidden, even for an account with a tester claim.
...
- The first-use English/Spanish notice prohibits names, diagnoses, contact details, addresses, school records and confidential team information; says AI can be wrong and is not emergency/medical/legal/disciplinary/safeguarding advice; explains Anthropic processing, temporary server retention, device-only saved guides and feedback. Nothing sends or publishes automatically.
- Saved guides stay local to the device until the user deletes the guide, signs out or completes local-account cleanup, clears app data, or uninstalls the app. Testing remains free, `paidEntitled` is false, and the server allows at most 10 unique requests per rolling 24 hours.
- This is locally prepared but not evidence of an enabled production beta. Before distribution, approve the provider/terms, policy and store disclosures; activate/verify TTL; configure spending limits/alerts; and complete the smoke/rollback gates in `docs/coach-ai-backend-enablement.md`.
