...
async function run() {
  ...
  const lobbyDirectory = fs.readFileSync(path.join(process.cwd(), 'app', '(games)', 'lobbies.tsx'), 'utf8');
  ...
  assert.doesNotMatch(lobbyHook, /createGameJoinCode|createGameLobby|releaseGameJoinCode/);
  assert.match(lobbyHook, /getGameJoinCodeForSession/);
  assert.match(lobbyHook, /leaveGameLobby/);
  ...
  assert.match(gamesScreen, /openGameDirectory/);
  assert.doesNotMatch(gamesScreen, /host:\s*["']1["']/);
  assert.match(lobbyDirectory, /createGameLobby/);
  assert.match(lobbyDirectory, /joinGameLobbyById/);
  assert.match(lobbyDirectory, /startAnotherTitle/);
