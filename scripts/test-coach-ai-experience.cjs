...
assert.equal(experience.resolveCoachAiFocusedInputScrollDelta(1280, 50, 1150, 32), 212);
assert.equal(experience.resolveCoachAiFocusedInputScrollDelta(1040, 50, 1150, 32), 0);
assert.equal(experience.resolveCoachAiFocusedInputScrollDelta(1080.2, 50.1, 1150, 32), 13);
assert.equal(experience.resolveCoachAiFocusedInputScrollDelta(Number.NaN, 50, 1150, 32), 0);
assert.equal(experience.resolveCoachAiFocusedInputScrollDelta(1280, 50, Number.NaN, 32), 0);
assert.equal(experience.resolveCoachAiVisibleKeyboardViewportBottom(1150, 100, 900), 1000);
assert.equal(experience.resolveCoachAiVisibleKeyboardViewportBottom(950, 100, 900), 950);
assert.equal(experience.resolveCoachAiVisibleKeyboardViewportBottom(2340, 0, 1150), 1150);
assert.equal(experience.resolveCoachAiVisibleKeyboardViewportBottom(Number.NaN, 100, 900), 1000);
assert.equal(experience.resolveCoachAiVisibleKeyboardViewportBottom(Number.NaN, Number.NaN, Number.NaN), 0);
assert.equal(experience.resolveCoachAiKeyboardViewportSupplement(34, 698, 438), 294);
assert.equal(experience.resolveCoachAiKeyboardViewportSupplement(34, 404, 438), 0);
assert.equal(experience.resolveCoachAiKeyboardViewportSupplement(34.3, 697.6, 438), 294);
assert.equal(experience.resolveCoachAiKeyboardViewportSupplement(Number.NaN, 698, 438), 0);
...
async function run() {
  ...
  assert.match(help, /onSelectionChange=\{multiline \|\| Platform\.OS === "android" \? revealInput : undefined\}/);
  assert.match(help, /scrollEnabled=\{multiline \? true : undefined\}/);
  assert.match(help, /onChangeText=\{\(text\) => \{[\s\S]*onChangeText\(text\);[\s\S]*if \(multiline \|\| Platform\.OS === "android"\) revealInput\(\)/);
  assert.match(help, /skillFocus[\s\S]*multiline onChangeText=\{setSituation\}[\s\S]*fields\.equipment[\s\S]*onChangeText=\{setEquipment\}/);
  ...
  assert.match(keyboard, /resolveCoachAiFocusedInputScrollDelta\([\s\S]*inputScreenY[\s\S]*inputHeight[\s\S]*revealOffset/);
  assert.match(keyboard, /findNodeHandle\(scrollRef\.current\)[\s\S]*UIManager\.measureInWindow\([\s\S]*resolveCoachAiVisibleKeyboardViewportBottom/);
  assert.match(keyboard, /resolveCoachAiVisibleKeyboardViewportBottom\([\s\S]*keyboardScreenY[\s\S]*scrollScreenY[\s\S]*scrollHeight/);
  assert.match(keyboard, /setKeyboardFrameSupplement\(resolveCoachAiKeyboardViewportSupplement\([\s\S]*visibleViewportBottom/);
  assert.match(keyboard, /revealTarget as TextInput\)\.measureInWindow\([\s\S]*scrollOffsetYRef\.current \+ revealDelta/);
  assert.match(keyboard, /targetStillFocused[\s\S]*TextInput\.State\.currentlyFocusedInput\(\)[\s\S]*isFocused/);
  assert.match(keyboard, /requestedInputRef\.current = input;[\s\S]*if \(keyboardVisibleRef\.current\) revealFocusedInput\(input\)/);
  assert.match(keyboard, /keyboardDidShow[\s\S]*revealFocusedInput\(requestedInputRef\.current\)/);
  assert.match(keyboard, /Platform\.OS === "android"[\s\S]*requestAnimationFrame\(performReveal\)/);
  assert.match(keyboard, /onScroll=\{\(event\) => \{[\s\S]*scrollOffsetYRef\.current = event\.nativeEvent\.contentOffset\.y/);
  ...
  assert.match(keyboard, /keyboardFrameSupplement > 0[\s\S]*pointerEvents="none" style=\{\{ height: keyboardFrameSupplement \}\}/);
  assert.match(keyboard, /targetIsFocused[\s\S]*!keyboardVisibleRef\.current \|\| !revealTarget \|\| !targetIsFocused/);
