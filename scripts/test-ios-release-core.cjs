...
const appleRevocation = read("functions", "src", "appleAuthorizationRevocation.ts");
const providerButtons = read("components", "FederatedAuthButtons.tsx");
const providerAvailability = read("hooks", "useAuthProviderAvailability.ts");
...
const firebaseEnvironment = read("config", "firebaseEnvironment.ts");
...
assert.equal(deleteScreen.includes("appleAuthorizationRef"), true);
assert.equal(deleteScreen.includes("appleAuthorizationCode"), true);
...
assert.equal(deletionFunction.includes("APPLE_REVOCATION_SECRET_NAMES"), true);
assert.equal(deletionFunction.includes("providerData.map((provider) => provider.providerId)"), true);
assert.equal(deletionFunction.indexOf("const appleRevocationRef = await ensureAppleAuthorizationRevoked") < deletionFunction.indexOf("await deleteAuthoredMessagesAndAudio"), true);
assert.match(appleRevocation, /APPLE_NATIVE_CLIENT_ID = 'com\.sidelinesocial\.app'/u);
...
assert.match(firebaseConfig, /Platform\.OS === "ios" \? "ios"[\s\S]*EXPO_PUBLIC_FIREBASE_PROJECT_ID:\s*process\.env\.EXPO_PUBLIC_FIREBASE_PROJECT_ID/);
assert.doesNotMatch(firebaseConfig, /resolveFirebaseClientConfig\(process\.env/);
assert.match(firebaseEnvironment, /appIdIos: "1:903830626771:ios:548f99d119be8948dfcf26"/);
assert.match(firebaseEnvironment, /platform === "ios" \? source\.appIdIos : source\.appIdAndroid/);
