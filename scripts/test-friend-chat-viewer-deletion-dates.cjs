...
assert.match(imageViewer, /loadFriendChatImageMedia/u, "viewer media uses the protected stable-cache loader");
assert.match(imageCacheService, /getFriendChatMediaDownloadUrl/u, "cache misses still require short-lived server authorization");
...
assert.match(chatScreen, /clearFriendChatImageCacheForMessages/u);
