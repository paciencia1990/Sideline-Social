...
export default function FriendChatManageScreen() {
  ...
  const title = access && user?.uid ? getConversationDisplayTitle(access.conversation, user.uid, t("chat.unnamedGroup")) : t("chat.conversationSettings");
    ...
    <ScreenWrapper>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        {isGroup ? <>
          ...
          <Card style={styles.section}><View style={styles.sectionHeading}><Users color={Colors.primary} size={20} /><Text style={styles.sectionTitle}>{t("chat.members", { count: members.length })}</Text></View>{members.map((member) => <TouchableOpacity key={member.userId} accessibilityLabel={t("chat.memberAccessibility", { name: member.displayNameSnapshot || t("chat.aFriend"), role: t(`chat.role.${member.role}`) })} accessibilityRole="button" disabled={member.userId === user?.uid} onPress={() => confirmMemberAction(member)} style={styles.memberRow}><View style={styles.avatar}><Text style={styles.avatarText}>{member.displayNameSnapshot.slice(0, 1).toUpperCase()}</Text></View><View style={styles.grow}><Text style={styles.memberName}>{member.displayNameSnapshot || t("chat.aFriend")}{member.userId === user?.uid ? ` ${t("chat.you")}` : ""}</Text><Text style={styles.role}>{t(`chat.role.${member.role}`)}</Text></View>{member.role !== "member" ? <Shield color={Colors.accentGold} size={17} /> : null}</TouchableOpacity>)}</Card>
