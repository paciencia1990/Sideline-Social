...
export default function ParentPrivateTeamMessageScreen() {
  ...
  return <ScreenWrapper><ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled"><View style={styles.header}><TouchableOpacity accessibilityLabel={t("teamMessages.back")} accessibilityRole="button" onPress={() => router.replace(`/teams/${teamId}` as never)} style={styles.back}><ArrowLeft color={Colors.textHeading} size={22} /></TouchableOpacity><Text accessibilityRole="header" style={styles.title}>{t("teamMessages.title")}</Text></View><PrivateTeamMessageThread conversationId={conversationId} role="parent" /></ScrollView></ScreenWrapper>;
}
function normalize(value?: string | string[]) { return Array.isArray(value) ? value[0] ?? "" : value ?? ""; }
const styles = StyleSheet.create({ content: { gap: Spacing.md, padding: Spacing.lg, paddingBottom: Spacing.xxl }, header: { alignItems: "center", flexDirection: "row", gap: Spacing.sm }, back: { alignItems: "center", height: 44, justifyContent: "center", width: 44 }, title: { color: Colors.textHeading, flex: 1, fontFamily: Typography.heading, fontSize: 26 } });
